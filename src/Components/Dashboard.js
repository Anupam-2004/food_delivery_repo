...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        console.log("Restaurant Sales Overview:", response.data);

        const data = Array.isArray(response.data) ? response.data : [];

        setOverview({
          labels: data.map((item) => item?.date || ""),
          totalOrders: data.map((item) => Number(item?.totalOrders) || 0),
          completed: data.map((item) => Number(item?.completed) || 0),
          cancelled: data.map((item) => Number(item?.cancelled) || 0),
        });
      })
      .catch((error) => {
        console.error("Restaurant sales overview error:", error);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8090/api/orders/analytics/sales-overview")
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8090/api/orders/analytics/revenue/monthly")
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8090/api/orders/analytics/revenue/daily")
      .then((response) => {
        console.log("Revenue Analytics:", response.data);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8090/api/orders/analytics/recent-orders")
      .then((response) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setTotalRestaurants(Number(response.data?.totalRestaurents) || 0);
      })
      .catch((error) => {
                            ...
                            <tr key={order?._id || order?.id || index}>
                              <td>
                                <span className="order-id">
                                  {getSafeText(order?._id || order?.id)}
                                </span>
                              </td>
                              <td>
                                {getSafeText(
                              ...
                              <td>
                                <div className="customer-cell">
                                  <div className="customer-avatar">
                                    {getSafeText(order?.userId?.firstName, "U")}
                                  </div>
                                  <span>
                                    {getSafeText(
                              ...
                              <td>
                                <strong>
                                  ₹{getSafeText(order?.totalAmount, "0")}
                                </strong>
                              </td>

                              <td>
                                <span>
                                  {getSafeText(order?.paymentStatus, "N/A")}
                                </span>
                              </td>
                            </tr>
                          ))
                ...
                {displayedRestaurants.length > 0 ? (
                  displayedRestaurants.map((restaurant, index) => (
                    ...
                    >
                      <img
                        src={
                          restaurant?.image || "/REStaurent/inner-view copy.jpg"
                        }
                        ...
                        salesOverView.map((item, index) => (
                          <li key={item?._id || index}>
                            <span
                              ...
                              style={{
                                backgroundColor:
                                  salesBreakdown[index % salesBreakdown.length]
                                    .color,
                              }}
                            />

                            <span className="legend-label">
                              {getSafeText(item?.timeline, "Sales")}
                            </span>
